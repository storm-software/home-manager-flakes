...
# Optional paid API credentials can expand routing beyond models covered by
# enrolled subscriptions. They are not needed for ChatGPT OAuth.
...
path.write_text(re.sub(
    r"(?m)^\s*(?:export\s+)?OPENAI_API_(?:KEY|TOKEN)=.*(?:\n|$)",
    "",
    content,
))
PY
